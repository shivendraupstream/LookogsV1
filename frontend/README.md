...
A self-hosted log management platform with core APM metrics — built to
replace AppSignal's logging + basic monitoring subscription for
Upstream Tech.
...
```
Paste the API key into `backend/.env` as `LOOKOGS_API_KEY`, restart.

### 11. Send a test log
```cmd
curl -X POST http://localhost:3000/api/v1/ingest -H "Content-Type: application/json" -H "x-api-key: YOUR_KEY" -d "{\"logs\": [{\"message\": \"Setup complete\", \"severity\": \"INFO\", \"eventTime\": \"2026-08-12T00:00:00Z\"}]}"
```
...
See **`INTEGRATION.md`**. The client is a published, installable
package — no manual file copying:
```
npm install github:shivendraupstream/lookogs-client
```
...
**Two known, documented gaps from that pass:**
- `Source` has no `format` column in the actual database — ingest
  format is set per-request via the `X-Log-Format` header, not a
  stored per-source default
- S3 archival code is written but inactive, pending real AWS
  credentials

**Beyond the original plan:**
- A working **Dashboard** page: total logs, error rate %, average
  response time, and a throughput/severity chart — real APM-style
  metrics, not just a raw log list
- `lookogs-client` extracted as its own installable package (Node/
  Fastify and browser variants)
...
**Phase 6+ (M4) — intentionally not started:**
- Alerting/triggers (no error rate hits with no notifications)
